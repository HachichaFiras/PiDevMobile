...
import com.esprit.semer.services.MessageService;
...
public class DrSbaitso {
  ...
  List<Message> all = MessageService.getInstance().getAllMessages(idd,SessionManager.getId() );
            for(Message m :all)
            {
                if(m.getSource().getId()==SessionManager.getId())
                {
        ...
        ActionListener askEvent = (e) -> {
            ...
            if (t1.length() > 0) {
                ...
                MessageService.getInstance().addMsg(new Message(u,u2, t1));
            }
        };
